...
SKILL_USAGE_BLOCK = (
    ...
    "not your limits. When a command doesn't directly support what the user asked:\n"
    "1. **Decompose the problem.** Break large queries into multiple smaller calls. "
    "   Example: 'monthly spend for last year' → call the report command once with "
    "   a large --days value and --by-day, then aggregate the daily rows into months yourself.\n"
    "2. **Combine data across calls.** Run the same command for different campaigns, "
    "   date ranges, or platforms, then merge and summarize the results.\n"
    "3. **Use what you have creatively.** If a CLI has --days but no --start-date, "
    "   compute the right --days value from today's date. If it has --by-day, use that "
    "   to get granular data and roll it up yourself.\n"
    "4. **Post-process results.** You can do math, filtering, sorting, and formatting "
    "   on tool results. Present tables, calculate totals, compute percentages, "
    "   and derive insights — don't just echo raw JSON back.\n"
    "5. **Never say 'the tool doesn't support X' as a final answer.** Always attempt "
    "   a workaround first. Only mention tool limitations as context while presenting "
    "   the best answer you can assemble.\n\n"
    "### Chaining Rules\n"
